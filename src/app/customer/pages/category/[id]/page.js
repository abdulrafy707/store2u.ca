...
const CategoryPage = () => {
  ...
  return (
    <div className="container mx-auto px-4 py-8 bg-white">
      <h2 className="text-2xl font-semibold mb-6">{category?.name} Subcategories</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
        {subcategories.map((subcategory, index) => (
          <motion.div
            ...
            transition={{ duration: 0.3 }}
            style={{ minHeight: '200px' }} // Adjust the minHeight value for better card height
          >
            {subcategory.imageUrl ? (
              <motion.img
                src={`https://data.tascpa.ca/uploads/${subcategory.imageUrl}`}
                alt={subcategory.name}
                className="w-full h-40 object-cover mb-2 rounded" // Adjusted image height
                whileHover={{ scale: 1.1 }}
                transition={{ duration: 0.3 }}
                onError={(e) => {
                  e.target.onerror = null; 
                  e.target.src = '/fallback-image.jpg'; // Replace with your fallback image path
                }}
              />
            ) : (
              <img
            ...
            {/* <p className="text-gray-500">{subcategory.description || 'No description available'}</p> */}
          </motion.div>
        ))}
      </div>
    </div>
